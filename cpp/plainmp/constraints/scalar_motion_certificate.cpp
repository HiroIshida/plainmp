...
bool SphereCollisionCst::prepare_motion_certificate(const VectorInput& start,
  ...
  bounds.minimum_rate = rate_radius / motion_certificate_steps_;
...
bool SphereCollisionCst::is_valid_with_motion_certificate(
  ...
  post_update_kintree();
  ++motion_certificate_stats_[1];
  certified_radius = motion_bounds_ ? motion_bounds_->maximum_rate : 0;
  const bool valid = check_motion_envelope(certified_radius);
  if (certified_radius > 0) ++motion_certificate_stats_[2];
  return valid;
}
