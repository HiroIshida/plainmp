...
ob::PlannerStatus LightningRetrieveRepairWrap::solve(
  ...
  // Restart the Planner Input States so that the first start and goal state can
  // be fetched
  pis_.restart();
  ...
  if (smoothingEnabled_) {
    OMPL_INFORM(
        "LightningRetrieveRepair solve: Simplifying solution (smoothing)...");
    ompl::time::point simplifyStart = ompl::time::now();
    std::size_t numStates = primaryPath->getStateCount();
    psk_->simplify(*primaryPath, ptc);
    double simplifyTime =
        ompl::time::seconds(ompl::time::now() - simplifyStart);
    OMPL_INFORM(
        "LightningRetrieveRepair: Path simplification took %f seconds and "
        "removed %d states",
